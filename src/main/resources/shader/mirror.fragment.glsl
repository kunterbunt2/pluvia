...
//	vec2 distortedTexCoords = texture(u_dudvMapTexture,	vec2(v_texCoord0.x + u_moveFactor, v_texCoord0.y)).rg * 0.1;
//	distortedTexCoords = v_texCoord0 + vec2(distortedTexCoords.x, distortedTexCoords.y + u_moveFactor);
//	vec2 totalDistortion = (texture(u_dudvMapTexture, distortedTexCoords).rg * 2.0 - 1.0) * u_waveStrength /** clamp(waterDepth/100.0, 0.0, 1.0)*/;
