 ...
 * <p>
 * Performs the operations: {@code #add(int, Object)}, {@code get} and {@code #remove(int)} operations in in time <i>O(log(n))</i> and the {@code #contains(Object)} and {@link #remove(Object)} operations in time
 * <i>O(n)</i>, where <i>n</i> is the number of elements in the list.
 * <p>
...
public class UnsortedList<T> extends SortedList<T> {
	...
	private class UnsortedNode extends Node {
		...
		/**
		 * Compares this node with the given node - which must be an {@code UnsortedNode} instance in the same {@code UnsortedList}. The comparison is based on the current position of the nodes in the list.
		 * <p>
		...
		private int getIndexInList() {
			...
			final LinkedList<UnsortedNode>	path	= new LinkedList<>();
	...
	private static Comparator<Object>	DUMMY_COMPARATOR	= new Comparator<>() {
																@Override
	...
	/**
	 * Adds the given object to the end of this {@code UnsortedList}, which can be <code>null</code>.
	 *
	...
	/**
	 * Returns whether or not the given object whether or not the given object is present in this {@code UnsortedList}.
	 * <p>
	 * This implementation takes <i>O(n)</i> time, where <i>n</i> is the number of element in this list.
	 *
	 * @return <code>true</code> if the given object is present in this {@code UnsortedList} and <code>false</code> otherwise.
	 */
	@Override
	...
	/**
	 * Removes the first element in the list with the given value, if such a node exists, otherwise does nothing. Works in time <i>O(n)</i>, where <i>i</i> is the number of elements in this {@code UnsortedList}.
	 * <p>
